...
func Resolv(data []byte) ([]byte, []byte, int64, []byte,byte,uint64,[]byte,uint16,[]byte,uint16, error) {
    //txt数据，用户名,timestamp,point_to,Typeid,key_des,hash,tag,point_to_root(主贴),tag2
    ...
    if err != nil {
        return nil, nil, 0, nil, 0, 0, nil, 0, nil, 0, err
    }
    if version != 1 {
        return nil, nil, 0, nil, 0, 0, nil, 0, nil, 0, errors.New("unsupported version")
    }
    ...
    if domainLen == 0 || domainLen > maxDomainLen {
        return nil, nil, 0, nil, 0, 0, nil, 0, nil, 0, errors.New("invalid domain length")
    }
    ...
    if length > maxTextLen {
        return nil, nil, 0, nil, 0, 0, nil, 0, nil, 0, errors.New("text too large")
    }
    ...
    if pointLen > maxPointLen {
        return nil, nil, 0, nil, 0, 0, nil, 0, nil, 0, errors.New("point too long")
    }
    ...
    if cp != 0 {
        return nil, nil, 0, nil, 0, 0, nil, 0, nil, 0, errors.New("unsupported compression")
    }
    ...
    return txt, domain, timestamp, pointTo,typeID,key_des,thash,tag,point_To_root,tag2,nil
}